...
class album_Compose_train():
    def __init__(self):
        self.albumentations_transform = Compose([
          ...
          RandomCrop(32,32),
          HorizontalFlip(),
          CoarseDropout(max_holes=1, max_height=8, max_width=8, p=0.5, fill_value=(0.4914*255, 0.4822*255, 0.4471*255)),
          Normalize(mean=[0.4914, 0.4822, 0.4471],std=[0.2469, 0.2433, 0.2615]),
          ToTensorV2()
        ])
    def __call__(self,img):
        img = self.albumentations_transform(image=np.array(img))['image']
        return img
...
class dataset_cifar10:
    ...
    def data_summary_stats(self):
        ...
        print("Number of Images : ", total_data.shape)
        print("Dataset Mean : ", total_data.mean(axis=(0,1,2))/255)
        print("Dataset Standard Deviation : ", total_data.std(axis=(0,1,2))/255)
